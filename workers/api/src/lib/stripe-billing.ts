...
import { log } from "./log";
import { selectOne, tableColumns, type DbRow } from "./d1-helpers";
...
function checkoutIntegrationId(): string {
  ...
  return `fixmyiron_checkout_${suffix}`;
}
...
> {
  ...
  const existingSubs = await stripeForm(env, "GET", "/subscriptions", {
    customer: stripeCustomerId,
    status: "all",
    limit: 20,
  });
  const subs = Array.isArray(existingSubs.json.data)
    ? (existingSubs.json.data as Array<{ status?: string }>)
    : [];
  if (
    subs.some(
      (sub) =>
        typeof sub.status === "string" &&
        NON_TERMINAL_SUBSCRIPTION_STATUSES.has(sub.status),
    )
  ) {
    return {
      error:
        "This account already has an active subscription. Manage it in the billing portal.",
      status: 409,
...
function tierForSubscriptionStatus(
  env: Env,
  stripeStatus: string,
  ...
  return tierForStripePrice(env, priceId);
}
...
export async function upsertStripeIds(
  ...
  patch: {
    ...
    stripeSubscriptionCreated?: number | null;
  ...
  const columns = await tableColumns(env.DB, "subscriptions");
  const payload: DbRow = {
    ...
    stripe_price_id: patch.stripePriceId ?? null,
    tier: patch.tier ?? "free",
    status: patch.status ?? "canceled",
    stripe_status: patch.stripeStatus ?? patch.status ?? "canceled",
    current_period_end: patch.currentPeriodEnd ?? null,
    past_due_since: patch.pastDueSince ?? null,
    stripe_subscription_created: patch.stripeSubscriptionCreated ?? null,
    stripe_event_created: patch.stripeEventCreated ?? null,
    updated_at: now,
  };
  const entries = Object.entries(payload).filter(
    ([name, value]) => columns.has(name) && value !== undefined,
  );
  ...
  if (!existing) {
    ...
    try {
      await env.DB
        .prepare(`INSERT INTO subscriptions (${names}) VALUES (${placeholders})`)
  ...
  const sets = entries
    .filter(([name]) => name !== "customer_id" && name !== "user_id")
  ...
  const updateValues = entries
    ...
    .map(([, value]) => value);
  const updates = [
    `UPDATE subscriptions SET ${sets} WHERE customer_id = ?${updateValues.length + 1}`,
    `UPDATE subscriptions SET ${sets} WHERE user_id = ?${updateValues.length + 1}`,
  ];
...
export async function applyStripeSubscriptionObject(
  ...
  const stripeStatus =
    typeof object.status === "string" ? object.status : "canceled";
  ...
  const tier = tierForSubscriptionStatus(env, stripeStatus, priceId);
  ...
  await upsertStripeIds(env, customerId, {
    ...
    stripeSubscriptionCreated:
      typeof object.created === "number" ? object.created : null,
    stripeEventCreated: eventCreated ?? null,
  });
  return { customerId };
}
