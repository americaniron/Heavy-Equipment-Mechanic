...
import { BACKEND_REPAIR_DDL } from "../src/lib/backend-repair-ddl";
...
describe("operational schema DDL", () => {
  it("splits additive CREATE TABLE statements without DROP", () => {
    ...
    expect(statements.some((sql) => /CREATE TABLE IF NOT EXISTS diagnostic_messages\b/i.test(sql))).toBe(true);
    expect(statements.some((sql) => /CREATE TABLE IF NOT EXISTS troubleshooting_turns\b/i.test(sql))).toBe(true);
    expect(statements.some((sql) => /CREATE TABLE IF NOT EXISTS predictive_alerts\b/i.test(sql))).toBe(true);
