...
import {
  ...
  authSessions, sessionAccessTokens,
...
export interface IStorage {
  ...
  // Auth session token persistence (replaces in-memory Map for Cloud Run compatibility)
  createAuthSession(token: string, customerId: number, ttlMs?: number): Promise<void>;
  getCustomerIdByAuthToken(token: string): Promise<number | undefined>;
  deleteAuthSession(token: string): Promise<void>;

  // Per-chat-session access token persistence
  setSessionAccessToken(sessionId: number, token: string): Promise<void>;
  getSessionAccessToken(sessionId: number): Promise<string | undefined>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  // ===== Auth session persistence (Cloud Run compatible) =====
  // Replaces the in-memory `customerSessions` Map. Tokens are durable across
  // container restarts and visible to all instances of the service.

  async createAuthSession(token: string, customerId: number, ttlMs: number = 30 * 24 * 60 * 60 * 1000): Promise<void> {
    const expiresAt = new Date(Date.now() + ttlMs);
    await db.insert(authSessions).values({ token, customerId, expiresAt });
  }

  async getCustomerIdByAuthToken(token: string): Promise<number | undefined> {
    const [row] = await db
      .select()
      .from(authSessions)
  ...
  // ===== Per-chat-session access tokens (Cloud Run compatible) =====
  // Replaces the in-memory `sessionTokens` Map. One token per chat session.

  async setSessionAccessToken(sessionId: number, token: string): Promise<void> {
    await db
      .insert(sessionAccessTokens)
      .values({ sessionId, token })
      .onConflictDoUpdate({ target: sessionAccessTokens.sessionId, set: { token } });
  }

  async getSessionAccessToken(sessionId: number): Promise<string | undefined> {
    const [row] = await db
      .select()
