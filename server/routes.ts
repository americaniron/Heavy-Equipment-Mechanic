...
async function generateSessionToken(sessionId: number): Promise<string> {
  const token = crypto.randomBytes(32).toString("hex");
  await storage.setSessionAccessToken(sessionId, token);
  return token;
}

async function validateSessionAccess(req: any, sessionId: number): Promise<boolean> {
  const token = req.headers["x-session-token"] || req.query.token;
  if (!token) return false;
  const stored = await storage.getSessionAccessToken(sessionId);
...
async function requireAuth(req: Request, res: Response, next: NextFunction) {
  ...
  try {
    const customerId = await storage.getCustomerIdByAuthToken(token);
    if (!customerId) {
      return res.status(401).json({ error: "Authentication required" });
    }
...
export async function registerRoutes(
  ...
  app.post("/api/auth/register", async (req, res) => {
    try {
      ...
      await storage.createAuthSession(authToken, customer.id);
  ...
  app.post("/api/sessions", async (req, res) => {
    try {
      ...
      if (resolvedCustomerId) {
        ...
        const authenticatedCustomerId = authToken
          ? await storage.getCustomerIdByAuthToken(authToken)
          : null;
      ...
      const accessToken = await generateSessionToken(session.id);
  ...
  app.get("/api/sessions/:id", async (req, res) => {
    try {
      ...
      if (!(await validateSessionAccess(req, sessionId))) {
        return res.status(403).json({ error: "Access denied" });
      }
  ...
  app.post("/api/sessions/:id/message", async (req, res) => {
    try {
      ...
      if (useVoice && audio) {
        // Voice-to-text path was tied to a Replit-specific audio integration
        // that was never wired up properly in the original code. Disable for now;
        // re-enable by wiring `audio` (base64 webm/wav) through a real STT (e.g. OpenAI Whisper).
        return res.status(501).json({ error: "Voice transcription temporarily unavailable; please type your message." });
      }
