...
// Persistent customer auth tokens (replaces in-memory customerSessions Map for Cloud Run)
...
// Persistent chat-session access tokens (replaces in-memory sessionTokens Map for Cloud Run)
export const sessionAccessTokens = pgTable("session_access_tokens", {
  sessionId: integer("session_id").primaryKey().references(() => sessions.id, { onDelete: "cascade" }),
